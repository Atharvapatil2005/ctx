...
public final class MemoryClientTest {
    ...
    private static void normalizesSetupJsonAsInitStatus() {
        MemoryClient client = MemoryClient.withTransport(new FakeTransport(
                "local-cli",
                "{\"schema_version\":1,\"data_root\":\"/tmp/ctx\",\"mode\":\"ready\",\"indexed_items\":9,"
                        + "\"catalog\":{\"cataloged_sessions\":1},\"import\":{\"resume\":false,\"totals\":{}},"
                        + "\"network_required\":false}"));

        InitResponse response = client.init(MemoryOptions.init().catalogOnly(true));
    ...
    private static void wrapsRawStatusAsTypedEnvelope() {
        ...
        assertEquals(Integer.valueOf(2), MemoryValue.integer(response.asMap().get("status") instanceof Map
    ...
    private static void normalizesRawShowAndLocateResponses() {
        ...
        responses.put("locateEvent", "{"
                ...
                + "}");
        MemoryClient client = MemoryClient.withTransport(new FakeTransport("local-cli", responses));
    ...
    private static void buildsSearchCommand() {
        FakeTransport transport = new FakeTransport(
                "local-cli",
                "{\"schema_version\":1,\"query\":\"memory\",\"results\":[]}");
        MemoryClient client = MemoryClient.withTransport(transport);

        client.search(MemoryOptions.search().query("memory").term("ctx").limit(5).refresh("off"));

        assertEquals("search", transport.lastOperation.name());
        assertContainsInOrder(transport.lastOperation.args(), "search", "memory", "--json");
    ...
    private static void hostedIsExplicitlyUnsupported() {
        MemoryClient client = MemoryClient.hosted(HostedConfig.builder().baseUrl("https://ctx.example.invalid").build());
        try {
            client.status();
            throw new AssertionError("expected hosted placeholder failure");
        } catch (CtxMemoryException.Unsupported error) {
    ...
    private static final class FakeTransport implements MemoryTransport {
        private final String name;
        private final String response;
        private final Map<String, String> responses;
        private MemoryOperation lastOperation;
