...
public class MemoryEnvelope {
    ...
    MemoryEnvelope(Map<String, Object> canonical) {
        this.contractVersion = MemoryValue.string(canonical.get("contractVersion"));
        Integer version = MemoryValue.integer(canonical.get("schemaVersion"));
        this.schemaVersion = version == null ? SCHEMA_VERSION : version.intValue();
        this.operation = MemoryValue.string(canonical.get("operation"));
        this.backend = new Backend(MemoryValue.objectAt(canonical, "backend"));
        ...
        for (Map.Entry<String, Object> entry : canonical.entrySet()) {
            if (!isCommonField(entry.getKey())) {
                payloadFields.put(entry.getKey(), MemoryValue.copy(entry.getValue()));
            }
        }
        this.fields = Collections.unmodifiableMap(payloadFields);
        this.envelope = MemoryValue.copyObject(canonical);
    }

    MemoryEnvelope(String operation, Backend backend, Map<String, Object> fields) {
    ...
    static MemoryEnvelope wrap(String operation, Backend backend, Map<String, Object> raw) {
        return new MemoryEnvelope(normalize(operation, backend, raw));
    }
    ...
    static Map<String, Object> normalize(String operation, Backend backend, Map<String, Object> raw) {
        if (CONTRACT_VERSION.equals(raw.get("contractVersion"))) {
            return MemoryValue.copyObject(raw);
        }

        Map<String, Object> camel = new LinkedHashMap<>(MemoryValue.camelizeObject(raw));
    ...
    private static Map<String, Object> buildCanonical(
        ...
        return MemoryValue.copyObject(canonical);
    }
    ...
    private static Map<String, Object> eventResult(Map<String, Object> camel) {
        ...
        if (out.get("source") == null) {
            Map<String, Object> event = MemoryValue.objectOrNull(camel.get("event"));
            if (event != null) {
