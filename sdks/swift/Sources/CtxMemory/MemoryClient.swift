...
public struct MemoryClient: Sendable {
    ...
    public static func hosted(_ config: HostedConfig = HostedConfig()) -> MemoryClient {
        MemoryClient(backend: .hosted(config))
    }
    ...
    public func initMemory(_ options: InitOptions = InitOptions()) throws -> InitResponse {
    ...
    public func importMemory(_ options: ImportOptions = ImportOptions()) throws -> ImportResponse {
        var arguments = ["import", "--json"]
        appendOption(&arguments, "--progress", options.progress)
        appendImportOptions(&arguments, options)
        return try ImportResponse(envelope: localEnvelope(operation: .importMemory, arguments: arguments))
    }
    ...
    public func errorEnvelope(for error: CtxMemorySDKError, operation: MemoryOperation = .error) -> MemoryEnvelope {
        let backendValue: MemoryBackend?
        switch backend {
        case let .local(adapter):
            backendValue = adapter.backend
        case let .hosted(config):
            backendValue = MemoryBackend(kind: "hosted", baseURL: config.baseURL?.absoluteString)
        }
        return MemoryEnvelope(operation: operation, backend: backendValue, error: error.contractError)
    }

    private func localEnvelope(operation: MemoryOperation, arguments: [String]) throws -> MemoryEnvelope {
        switch backend {
        case let .local(adapter):
    ...
    private func decodeJSONObject(_ data: Data) throws -> JSONValue {
        do {
            ...
            guard case .object = value else {
                throw CtxMemorySDKError(code: .decodeError, message: "ctx returned a non-object JSON value")
            }
            return value
        } catch let error as CtxMemorySDKError {
            throw error
        } catch {
    ...
    private func makeEnvelope(operation: MemoryOperation, backend: MemoryBackend, raw: JSONValue) throws -> MemoryEnvelope {
        switch operation {
        case .status:
            return MemoryEnvelope(
                operation: operation,
                backend: backendWithRawDataRoot(backend, raw),
                status: try decodeTyped(normalizeStatus(raw), as: MemoryStatus.self, context: "status")
            )
        case .initialize:
            return MemoryEnvelope(
                operation: operation,
        ...
        case .importMemory, .sync:
            return MemoryEnvelope(
                operation: operation,
                backend: backendWithRawDataRoot(backend, raw),
                importResult: try decodeTyped(normalizeImport(raw), as: MemoryImportResult.self, context: "import")
            )
        case .search:
            return MemoryEnvelope(
                operation: operation,
                backend: backendWithRawDataRoot(backend, raw),
                search: try decodeTyped(normalizeSearch(raw), as: MemorySearchResult.self, context: "search")
            )
        case .showEvent:
            return MemoryEnvelope(
                operation: operation,
                backend: backendWithRawDataRoot(backend, raw),
                event: try decodeTyped(normalizeEvent(raw), as: MemoryEventResult.self, context: "event")
            )
        case .showSession:
            return MemoryEnvelope(
                operation: operation,
                backend: backendWithRawDataRoot(backend, raw),
                session: try decodeTyped(normalizeSession(raw), as: MemorySessionResult.self, context: "session")
            )
        case .locateEvent, .locateSession:
            return MemoryEnvelope(
                operation: operation,
                backend: backendWithRawDataRoot(backend, raw),
                location: try decodeTyped(raw.camelizedPublicJSON().droppingNulls(), as: MemoryLocationResult.self, context: "location")
            )
        case .error:
            throw CtxMemorySDKError(code: .invalidRequest, message: "error is not a local CLI operation")
        }
    }
}
...
private func requireID(_ name: String, _ id: String) throws {
    if id.isEmpty {
        throw CtxMemorySDKError(code: .invalidRequest, message: "\(name) is required")
    }
}
...
private func backendWithRawDataRoot(_ backend: MemoryBackend, _ raw: JSONValue) -> MemoryBackend {
    guard backend.dataRoot == nil else {
        return backend
    }
    let dataRoot = raw["data_root"]?.stringValue ?? raw["dataRoot"]?.stringValue
    return MemoryBackend(kind: backend.kind, dataRoot: dataRoot, baseURL: backend.baseURL)
}
