...
public struct LocalCLIAdapter: Sendable {
    ...
    public var backend: MemoryBackend {
        MemoryBackend(kind: "local", dataRoot: dataRoot)
    }

    public func execute(_ arguments: [String]) throws -> Data {
        guard !ctxPath.isEmpty else {
            throw CtxMemorySDKError(code: .invalidRequest, message: "local ctx CLI path is empty")
        }
    ...
    private func commandError(result: CommandResult, arguments: [String]) -> CtxMemorySDKError {
        let stdout = String(data: result.stdout, encoding: .utf8) ?? ""
        let stderr = String(data: result.stderr, encoding: .utf8) ?? ""
        let firstStderrLine = stderr.split(whereSeparator: \.isNewline).first.map(String.init)
        return CtxMemorySDKError(
            code: .adapterError,
