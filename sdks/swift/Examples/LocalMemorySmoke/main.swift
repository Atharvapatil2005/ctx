...
@main
struct LocalMemorySmoke {
    static func main() throws {
        let config = SmokeConfig(arguments: Array(CommandLine.arguments.dropFirst()))
        let client = config.makeClient()

        let status = try client.status()
        let initialized = try client.initMemory(InitOptions(catalogOnly: true))
        let imported = try client.importMemory(ImportOptions(provider: "codex", resume: true))
...
private struct SmokeConfig {
    ...
    var useRealCtx: Bool {
        arguments.contains("--real") || ProcessInfo.processInfo.environment["CTX_MEMORY_SMOKE_REAL"] == "1"
    }

    func makeClient() -> MemoryClient {
        if useRealCtx {
        ...
        return MemoryClient(
            adapter: LocalCLIAdapter(
                dataRoot: "/tmp/ctx-swift-local-memory-smoke",
...
private final class FakeSmokeRunner: CommandRunner, @unchecked Sendable {
    func run(_ request: CommandRequest) throws -> CommandResult {
        let arguments = request.arguments.filter { $0 != "--data-root" && $0 != "/tmp/ctx-swift-local-memory-smoke" }
        switch Array(arguments.prefix(2)) {
        case ["status", "--json"]:
            return CommandResult(stdout: #"{"initialized":true,"local_only":true,"data_root":"/tmp/ctx-swift-local-memory-smoke","indexed_items":3,"indexed_sources":1,"cataloged_sessions":1,"pending_catalog_sessions":0,"failed_catalog_sessions":0,"stale_catalog_sessions":0}"#)
        case ["setup", "--json"]:
            return CommandResult(stdout: #"{"schema_version":1,"data_root":"/tmp/ctx-swift-local-memory-smoke","mode":"catalog_only","indexed_items":3,"network_required":false}"#)
        case ["import", "--json"]:
