...
internal static class Program
{
    ...
    private static async Task BuildsOperationArguments()
    {
        ...
        await client.ImportMemoryAsync(new ImportOptions { Provider = "codex", Resume = true });
    ...
    private static async Task WrapsShowAndLocate()
    {
        ...
        await ThrowsAsync<CtxMemoryValidationException>(() => client.ShowEventAsync(""));
        await ThrowsAsync<CtxMemoryValidationException>(() => client.LocateSessionAsync(new SessionLookupOptions { Provider = "codex" }));
    }
    ...
    private static async Task ReportsVersioning()
    {
        ...
        Equal(CtxMemoryVersions.ContractVersion, version.ApiVersion);
        Equal("test", version.Transport);
        Equal("ctx 1.2.3", version.CtxVersion ?? "");

        var versioning = await client.VersioningAsync();
        Equal(CtxMemoryVersions.SdkVersion, versioning.SdkVersion);
    }

    private static Task HostedPlaceholderError()
    {
        var client = MemoryClient.Hosted(new HostedMemoryConfig("https://ctx.example.invalid"));
    ...
    private static Task UsesMemoryV1ErrorCodes()
    {
        Equal("invalid_request", new CtxMemoryValidationException("bad").Code);
        Equal("decode_error", new CtxMemoryProtocolException("bad").Code);
        Equal("adapter_error", new CtxMemoryCliException("bad", ["ctx"], 1, "", "").Code);
        Equal("timeout", new CtxMemoryCliException("timeout", ["ctx"], -1, "", "", code: "timeout", retryable: true).Code);
        Equal(true, new CtxMemoryCliException("timeout", ["ctx"], -1, "", "", code: "timeout", retryable: true).Retryable);
        Equal("unknown", new CtxMemoryException("unknown").Code);
        ...
        foreach (var path in Directory.EnumerateFiles(fixtures, "*.json").Order())
        {
            ...
            switch (operation)
            {
                ...
                case "sync":
                    if (operation == "import")
                    {
                        _ = (await ClientFor(node["import"]).ImportMemoryAsync()).Import.Totals.ImportedEvents;
                    }
                    else
                    {
    ...
    {
        return new MemoryClient(new RecordingTransport(Clone(payload)?.ToJsonString() ?? "{}"));
    }
    ...
    private sealed class RecordingTransport : IMemoryTransport
    {
