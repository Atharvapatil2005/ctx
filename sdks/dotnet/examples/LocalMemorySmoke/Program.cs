...
var imported = await client.ImportMemoryAsync(new ImportOptions { Provider = "codex", Resume = true });
...
static MemoryClient CreateClient()
{
    var ctxPath = Environment.GetEnvironmentVariable("CTX_MEMORY_CTX");
    if (!string.IsNullOrWhiteSpace(ctxPath))
    {
        return MemoryClient.Local(new LocalMemoryConfig
        {
            CtxBinary = ctxPath,
            DataRoot = Environment.GetEnvironmentVariable("CTX_MEMORY_DATA_ROOT") ?? "/tmp/ctx-memory-dotnet-smoke",
            Timeout = TimeSpan.FromSeconds(30)
        });
    }

    return new MemoryClient(new FakeMemoryTransport());
}
...
internal sealed class FakeMemoryTransport : IMemoryTransport
{
    private const string DataRoot = "/tmp/ctx-memory-dotnet-smoke";
        ...
        CancellationToken cancellationToken = default)
    {
        return Task.FromResult(operation switch
        {
            ...
            _ => throw new CtxMemoryValidationException($"fake transport does not implement {operation}")
        });
    }
    ...
    private static JsonObject ShowSession(string sessionId)
    {
        ...
        payload["session"] = new JsonObject
        {
            ["ctx_session_id"] = sessionId,
            ["provider"] = "codex",
            ["title"] = "LocalMemorySmoke fixture"
        };
        payload["events"] = new JsonArray();
        payload["source"] = Source();
        payload["mode"] = "lite";
