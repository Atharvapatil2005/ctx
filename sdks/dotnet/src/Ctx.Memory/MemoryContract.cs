...
internal static class MemoryContract
{
    public static JsonObject Envelope(string operation, JsonObject backend, string payloadName, JsonNode? payload)
    {
        var result = new JsonObject
        {
            ["contractVersion"] = CtxMemoryVersions.ContractVersion,
            ["schemaVersion"] = CtxMemoryVersions.SchemaVersion,
            ["operation"] = operation,
    ...
    public static void EnsureSupportedSchema(JsonObject raw, string operation)
    {
        ...
        if (schema is not null && schema != CtxMemoryVersions.SchemaVersion)
        {
