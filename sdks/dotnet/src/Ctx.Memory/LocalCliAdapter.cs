...
public sealed class LocalCliAdapter : IMemoryTransport
{
    ...
    {
        Config = config ?? new LocalMemoryConfig();
    }

    public string Name => "local-cli";
    public LocalMemoryConfig Config { get; }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (stdout.Length == 0)
        {
            throw new CtxMemoryProtocolException(
    ...
    private async Task<CommandResult> ExecuteAsync(IReadOnlyList<string> args, CancellationToken cancellationToken)
    {
        ...
        {
            throw new CtxMemoryValidationException("local ctx CLI path is empty");
        }
        ...
        {
            throw new CtxMemoryCliException("failed to execute ctx CLI", command, -1, "", ex.Message, innerException: ex);
        }
        ...
        catch (OperationCanceledException ex)
        {
            ...
            throw new CtxMemoryCliException("ctx CLI timed out", command, -1, stdout, stderr, code: "timeout", retryable: true, innerException: ex);
        }
        ...
        {
            throw new CtxMemoryCliException("ctx CLI command failed", command, process.ExitCode, outText, errText);
        }
