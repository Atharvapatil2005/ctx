...
public abstract record MemoryResponse
{
    ...
    protected MemoryResponse(JsonObject envelope)
    {
        _json = JsonHelpers.CloneObject(envelope);
        ContractVersion = JsonHelpers.GetString(envelope, "contractVersion") ?? CtxMemoryVersions.ContractVersion;
        SchemaVersion = JsonHelpers.GetInt(envelope, "schemaVersion") ?? CtxMemoryVersions.SchemaVersion;
        Operation = JsonHelpers.GetString(envelope, "operation") ?? "";
        Backend = MemoryBackend.FromJson(envelope["backend"] as JsonObject);
    }

    public string ContractVersion { get; }
    public int SchemaVersion { get; }
    public string Operation { get; }
    public MemoryBackend Backend { get; }

    public JsonObject ToJsonObject() => JsonHelpers.CloneObject(_json);
}

public sealed record StatusResponse : MemoryResponse
{
    internal StatusResponse(JsonObject envelope)
        : base(envelope)
    {
        Status = MemoryStatus.FromJson(envelope["status"] as JsonObject);
    }

    public MemoryStatus Status { get; }
}

public sealed record InitResponse : MemoryResponse
{
...
public sealed record SourcesResponse : MemoryResponse
{
...
public sealed record ImportResponse : MemoryResponse
{
...
public sealed record SearchResponse : MemoryResponse
{
...
public sealed record ShowEventResponse : MemoryResponse
{
...
public sealed record ShowSessionResponse : MemoryResponse
{
...
public sealed record LocateEventResponse : MemoryResponse
{
...
public sealed record LocateSessionResponse : MemoryResponse
{
...
public sealed record MemoryBackend
{
    private readonly JsonObject _json;

    private MemoryBackend(JsonObject json)
    {
    ...
    internal static MemoryBackend FromJson(JsonObject? json) => new(json ?? new JsonObject());
}
...
public sealed record MemoryStatus
{
    ...
    private MemoryStatus(JsonObject json)
    {
        _json = JsonHelpers.CloneObject(json);
    ...
    internal static MemoryStatus FromJson(JsonObject? json) => new(json ?? new JsonObject());
}
...
public sealed record Freshness
{
    ...
    private Freshness(JsonObject json)
    {
        ...
        Totals = json["totals"] is JsonObject totals ? Ctx.Memory.Totals.FromJson(totals) : null;
        Error = JsonHelpers.GetString(json, "error");
    }
...
public sealed record SearchResult
{
    ...
    private SearchResult(JsonObject json)
    {
        ...
        Freshness = Ctx.Memory.Freshness.FromJson(json["freshness"] as JsonObject);
...
public sealed record EventResult
{
    ...
    {
        _json = JsonHelpers.CloneObject(json);
        Event = MemoryEvent.FromJson(json["event"] as JsonObject);
        Events = JsonHelpers.GetObjectArray(json, "events", MemoryEvent.FromJsonRequired);
        Source = SourceLocation.FromJson(json["source"] as JsonObject);
    }

    public MemoryEvent? Event { get; }
    public IReadOnlyList<MemoryEvent> Events { get; }
...
public sealed record MemoryEvent
{
    ...
    private MemoryEvent(JsonObject json)
    {
        _json = JsonHelpers.CloneObject(json);
        CtxEventId = JsonHelpers.GetString(json, "ctxEventId");
        CtxSessionId = JsonHelpers.GetString(json, "ctxSessionId");
    ...
    internal static MemoryEvent? FromJson(JsonObject? json) => json is null ? null : new MemoryEvent(json);
    internal static MemoryEvent FromJsonRequired(JsonObject json) => new(json);
}
